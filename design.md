...
## RFC000: functional requirements

The dispatcher plays a simple yet essential role in the EOSC system: it facilitates communication between the centralized matchmaker and the various tools. 
Its responsibilities are intentionally limited primarily, the dispatcher's function is to launch tools and manage communication, but it does not perform any additional processing or computation.

The RO-Crate serves as a non-functional entity in this context. 
It can be considered a pre-processing artifact, preparing or encapsulating messages received from upstream components before they are dispatched. 
Importantly, the dispatcher itself does not interpret or modify the RO-Crate; it simply passes it along according to the communication protocol.

From this perspective, the role and goal of the dispatcher (or more generally, the data player) are clear: it acts as a protocol-driven intermediary that abstracts the communication streams of the tools. 
By doing so, it decouples the matchmaker from the internal implementations of the tools, ensuring a consistent, reliable, and maintainable communication layer within the EOSC ecosystem.

Key functional principles:

- Protocol abstraction: the dispatcher defines a standard interface and communication protocol for interacting with any tool.
- Tool launching: the dispatcher is responsible for starting tools in response to upstream instructions.
- Message forwarding: it reliably forwards messages or RO-Crates between the matchmaker and tools without modifying their contents.
- Decoupling: upstream components remain independent of the internal details of individual tools.

This design ensures that the dispatcher is lightweight, focused, and maintainable, providing a clear separation of concerns within the EOSC system.
